 ...
 */
@Singleton
public class ListIconsImpl implements ListIcons
{
    ...
    @Inject
    ListIconsImpl(final AbiquoContext context)
    {
        this.context = context;
    }

    @Override
    public Iterable<Icon> execute()
    {
        IconsDto result = context.getApi().getConfigClient().listIcons();
        return wrap(context, Icon.class, result.getCollection());
    }

    @Override
    public Iterable<Icon> execute(final Predicate<Icon> selector)
    {
